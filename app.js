...
const DEF={employeeName:'',workplaceName:'가축위생방역지원본부 충북도본부',workplaceAddress:'',lat:'',lng:'',radius:100,workTime:'08:50',offTime:'18:00',remindAgain:true,days:[1,2,3,4,5]};
...
function fill(){const s=settings();$('employee-name').value=s.employeeName||'';$('workplace-name').value=s.workplaceName||DEF.workplaceName;$('workplace-address').value=s.workplaceAddress||'';$('lat').value=s.lat||'';$('lng').value=s.lng||'';$('radius').value=s.radius||100;$('work-time').value=s.workTime||'08:50';$('off-time').value=s.offTime||'18:00';$('remind-again').checked=s.remindAgain!==false;document.querySelectorAll('.day').forEach(x=>x.checked=(s.days||DEF.days).map(Number).includes(Number(x.value)));$('hello').textContent=s.employeeName?`${s.employeeName}님, 오늘도 인증을 확인해 주세요.`:'설정에서 이름과 근무지를 등록해 주세요.';updateNotifyStatus();updateNext()}
function saveForm(){const s={employeeName:$('employee-name').value.trim(),workplaceName:$('workplace-name').value.trim(),workplaceAddress:$('workplace-address').value.trim(),lat:$('lat').value,lng:$('lng').value,radius:Number($('radius').value)||100,workTime:$('work-time').value||'08:50',offTime:$('off-time').value||'18:00',remindAgain:$('remind-again').checked,days:[...document.querySelectorAll('.day:checked')].map(x=>Number(x.value))};saveS(s);fill();$('settings-msg').textContent='저장 완료';toast('설정을 저장했습니다.')}
...
function certify(type){const s=settings();if(!s.lat||!s.lng){toast('먼저 설정에서 근무지 위치를 등록해 주세요.');showPage('settings');return}if(!navigator.geolocation)return toast('GPS를 사용할 수 없습니다.');const out=type==='in'?'gps-result':'gps-result-2';$(out).textContent='현재 위치 확인 중…';navigator.geolocation.getCurrentPosition(p=>{const d=Math.round(dist(Number(s.lat),Number(s.lng),p.coords.latitude,p.coords.longitude));if(d>Number(s.radius)){$(out).textContent=`인증 실패 · 근무지에서 ${d}m · 허용 ${s.radius}m`;return}const h=hist(),today=date();let r=h.find(x=>x.date===today);if(!r){r={date:today,employeeName:s.employeeName,workplaceName:s.workplaceName,workplaceAddress:s.workplaceAddress,checkIn:'',checkOut:'',inDistance:'',outDistance:''};h.unshift(r)}if(type==='in'){r.checkIn=time();r.inDistance=d}else{r.checkOut=time();r.outDistance=d}r.lat=p.coords.latitude.toFixed(7);r.lng=p.coords.longitude.toFixed(7);saveH(h.slice(0,500));localStorage.setItem(`certified_${today}_${type}`,'1');$(out).textContent=`${type==='in'?'출근':'퇴근'} 인증 완료 · ${s.workplaceAddress||s.workplaceName} · ${d}m · ${time()}`;renderToday();renderHistory();toast('인증이 완료되었습니다.')},()=>$(out).textContent='GPS 인증 실패 · 위치 권한을 확인해 주세요.',{enableHighAccuracy:true,timeout:15000,maximumAge:0})}
...
function renderHistory(){const h=hist();$('history-list').innerHTML=h.length?h.slice(0,30).map(r=>`<div class="record"><b>${r.date}</b><div>근무지: ${r.workplaceAddress||r.workplaceName||'미등록'}</div><div>출근: ${r.checkIn||'미인증'}${r.inDistance!==''?` · ${r.inDistance}m`:''}</div><div>퇴근: ${r.checkOut||'미인증'}${r.outDistance!==''?` · ${r.outDistance}m`:''}</div></div>`).join(''):'<p class="muted">아직 인증 기록이 없습니다.</p>'}
function csv(){const h=hist();if(!h.length)return toast('다운로드할 기록이 없습니다.');const rows=[['날짜','이름','근무지','주소','출근','출근거리','퇴근','퇴근거리'],...h.map(r=>[r.date,r.employeeName,r.workplaceName,r.workplaceAddress,r.checkIn,r.inDistance,r.checkOut,r.outDistance])];const blob=new Blob(['\ufeff'+rows.map(r=>r.map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(',')).join('\n')],{type:'text/csv;charset=utf-8'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='출퇴근인증기록.csv';a.click();URL.revokeObjectURL(u)}
...
function updateNext(){const s=settings(),d=new Date(),wd=d.getDay(),ok=(s.days||[]).map(Number).includes(wd);$('next-alarm').textContent=ok?`오늘 알림: 출근 ${s.workTime} / 퇴근 ${s.offTime} · 1분 후 재알림 ${s.remindAgain!==false?'켜짐':'꺼짐'}`:`오늘은 알림 요일이 아닙니다. (설정한 요일 기준)`}
async function sendScheduledNotification(type,k,repeat=0){
  const title=type==='in'?'⏰ 출근 인증 알림':'🏠 퇴근 인증 알림';
  const body=repeat?'⚠️ 아직 인증하지 않으셨다면 지금 출퇴근 인증을 해주세요.':(type==='in'?'출근 인증을 해주세요.':'퇴근 인증을 해주세요.');
  const tag=`attendance-${k}-${type}-${repeat}`;
  try{
  ...
  try{new Notification(title,{body,tag,renotify:true});return true}catch(e){return false}
}
function schedule(){
